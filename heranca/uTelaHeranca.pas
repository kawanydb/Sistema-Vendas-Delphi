...
begin
  ...
  if PosOrder > 0 then
  begin
    SQLSemOrder := Copy(SelectOriginal, 1, PosOrder - 1); //pega o sql antes de orderby
    OrderByClause := Copy(SelectOriginal, PosOrder, Length(SelectOriginal)); //pega somente o order by
  end
  else
  begin
  ...
  //muda para pesquisar se tiver maiúscula ou minúscula no texto
    ...
    QryListagem.Close;
    QryListagem.SQL.Clear;
    QryListagem.SQL.Add(SQLSemOrder + CondicaoSQL + ' ' + OrderByClause); //monta o sql dnv
    QryListagem.Open;
  ...
  begin
    ...
    //se o campo selecionado for statusId
      ...
      TipoCampo := QryListagem.Fields[I].DataType;  //guarda o tipo do campo

      //se o nome for pessoaDescricao
  ...
  begin
  //monta o sql e pesquisa
    CondicaoSQL := WhereOrAnd +
                   NomeCampo + ' LIKE ' +
                   QuotedStr('%' + mskPesquisar.Text + '%');
  end
  //verifica se for data
...
begin
  ...
  begin
    ...
    case Col.Field.DataType of
      // Inteiros → centralizado (IDs, códigos, etc)
      ftSmallint, ftInteger, ftWord, ftLargeint, ftAutoInc:
        Col.Alignment := taCenter;

      // Decimais / monetários → direita
