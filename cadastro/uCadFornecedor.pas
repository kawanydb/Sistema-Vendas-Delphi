...
type
  ...
  private
    ...
    function FormatarTelefone(const Valor: string): string;
    function ValidarCnpj(const Valor: string): Boolean;
  public
    { Public declarations }
  end;
...
begin
  ...
  if frmTelaHeranca.DocumentoJaCadastrado(
    edtCnpj.Text,
    QryListagem.FieldByName('fornecedorId').AsInteger) then
  begin
...
procedure TfrmCadFornecedorProduto.strngfldQryListagemcnpjGetText(Sender: TField; var Text: string; DisplayText: Boolean);
begin
  if DisplayText then
    Text := FormatarCNPJ(Sender.AsString)
  else
    Text := Sender.AsString;
end;

procedure TfrmCadFornecedorProduto.strngfldQryListagemtelefoneGetText(Sender: TField; var Text: string;
  DisplayText: Boolean);
begin
  inherited;
  inherited;
...
begin
  ...
  edtCnpj.Text := FormatarCNPJ(SomenteNumeros(edtCnpj.Text));
...
function TfrmCadFornecedorProduto.FormatarCNPJ(const CNPJ: string): string;
begin
  Result :=
    Copy(CNPJ,1,2) + '.' +
    Copy(CNPJ,3,3) + '.' +
...
function TfrmCadFornecedorProduto.FormatarTelefone(const Valor: string): string;
var
  Num, Texto: string;
begin
...
begin
  ...
  // bloqueia repetidos
  if Num = StringOfChar(Num[1], Length(Num)) then Exit;
  ...
  Soma := 0;
  for i := 1 to 12 do
    Soma := Soma + (Ord(Num[i]) - Ord('0')) * Peso[i];

  Resto := Soma mod 11;
  if Resto < 2 then Resto := 0 else Resto := 11 - Resto;

  if Resto <> (Ord(Num[13]) - Ord('0')) then Exit;
  ...
  if Resto <> (Ord(Num[14]) - Ord('0')) then Exit;
