...
type
  TfrmCadAcaoAcesso = class(TfrmTelaHeranca)
    ...
    fdtncfldQryListagemacaoAcessoId: TFDAutoIncField;
...
begin
//seleciona o registro na classe puxando pela qry e apaga
  Result:=False;
...
begin
//seleciona o registro e coloca informações nele
  if oAcaoAcesso.Selecionar(QryListagem.FieldByName('acaoAcessoId').AsInteger) then begin
...
begin
 //caso não existe, ele verifica se tá vazio, se estiver vazio ele coloca
 //como 0 senão ele converte o número p/ inteiro
  ...
  oAcaoAcesso.chave     :=edtChave.Text;
   ...
   //antes de executar na tela de herança verifica se a chave existe antes de gravar,
   //se existir ele abandona
